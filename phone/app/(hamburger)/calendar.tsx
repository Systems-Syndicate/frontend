...
const backendURL = process.env.EXPO_PUBLIC_API_URL || "http://172.16.10.240:3801";

const fetchEvents = async (): Promise<Event[]> => {
  try {
    const response = await fetch(`${backendURL}/events/123456`);
    if (!response.ok) {
...
const parseEventsToMarkedDates = (events: Event[]) => {
  ...
  events.forEach((event) => {
    const { begin, classification, description, end, location, name, uid } = event;
    const eventDate = begin.split(" ")[0];
    const dot = {
      key: uid,
      color: "gray",
...
const groupEventsByDate = (events: Event[]): EventsByDate => {
  return events.reduce < EventsByDate > ((acc, event) => {
    const dateString = CalendarUtils.getCalendarDateString(event.begin);
    if (!acc[dateString]) {
      acc[dateString] = [];
    }
          ...
          >
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                ...
                <Text>
                  {parseDateToTimeString(selectedEvent?.begin || "")} -{" "}
                  {parseDateToTimeString(selectedEvent?.end || "")}
                </Text>
                <Text>{selectedEvent?.description || "No summary available."}</Text>
                <TouchableOpacity
